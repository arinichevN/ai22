...
void aoidServer_sendAiI(Aoid *oid, Acpls *server, int *v1, size_t v1_len, int v2){
	size_t arr_len = v1_len + 3;
	int arr[arr_len];
	arr[0] = app_id;
	arr[1] = oid->id;
	...
	arr[arr_len - 1] = v2;
	if(acp_buildPackAi(server->acpl->buf, ACP_BUF_MAX_LENGTH, ACP_SIGN_RESPONSE, arr, arr_len)){
		acpls_beginWrite(server);
		return;
	}
	acpls_reset(server);
...
static int aoidServer_supportedSet(int command){
	switch(command){
...
static int aoidServer_supportedSS(int command){
	switch(command){
		case CMD_AOID_START: case CMD_AOID_STOP:
			return 1;
	}
	return 0;
}
...
void aoidServer_sendSupportedSet(Aoid *oid, Acpls *server){
	aoidServer_sendSupportedCommand(oid, server, aoidServer_supportedSet);
}
...
void aoidServer_sendSupportedSS(Aoid *oid, Acpls *server){
	aoidServer_sendSupportedCommand(oid, server, aoidServer_supportedSS);
}
