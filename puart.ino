...
#define FOREACH_DEVICE for(int i=0;i<len;i++){Device *device = &devices[i];
...
void puart_server(PUART *item, Device *devices, int len) {
  ...
  switch (item->state) {
    case READ_COMMAND:
      ...
      if (PUART_CMD_IS(PUART_CMD_SELECT) ) {
        item->state = READ_INT;
		      ...
		      if (PUART_CMD_IS(PUART_CMD_END) ) {
				item->selected = 0;
				FOREACH_DEVICE
					device->temperature.selected = 0;
					device->humidity.selected = 0;
				}
			  } else if (PUART_CMD_IS(PUART_CMD_SELECTED)) {
			    FOREACH_DEVICE
			    if (device->temperature.selected) {
			      SP(device->temperature.id); SWDE; SF;
			    }
			    if (device->humidity.selected) {
			      SP(device->humidity.id); SWDE; SF;
			    }
			  }
	...
	case READ_INT://Serial.println("READ_INT");	Serial.print("CMD: "); Serial.println(item->cmd);
		...
		if (PUART_CMD_IS(PUART_CMD_SELECT)) {
		  FOREACH_DEVICE
		  if (device->temperature.id == v) {
			  item->selected = 1;
		    device->temperature.selected = 1;
		    //Serial.print("channel selected ");	Serial.println(v);
		    break;
		  }
		  if (device->humidity.id == v) {
			  item->selected = 1;
		    device->humidity.selected = 1;
		    //Serial.print("channel selected ");	Serial.println(v);
		    break;
		  }
		}
